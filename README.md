...
- 文字用 **Switch 系统共享字体**，自带中日韩字形。
  想换字体就把一个 `font.ttf` 放进 `sdmc:/switch/MusicPlayer2/`，系统字体会留作兜底
...
屏幕上有三个自带说明的操作区，直接点就行：

- **左下角**十字按钮，与方向键一一对应
- **右下角**菱形按钮，与 ABXY 一一对应
- **歌词区左上角**两组"减 · 读数 · 加"：歌词偏移和音量

歌词区右上角还有三个小按钮：下载、"拖歌词是否带着进度走"、单栏/双栏切换。
右下角显示当前是第几首 / 共几首。
