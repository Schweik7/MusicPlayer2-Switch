...
### 3.7 自动更新分两步，而且替换必须赶在 romfsInit 之前

**挂着 romfs 的时候，正在运行的 NRO 文件是被 FS 层按住的**：删不掉、改不了名、
也打不开写。romfs 就挂在这个 NRO 文件里，所以只要 `romfsInit()` 生效着，
任何"替换自身"的尝试都会失败。

这一条卡了好几轮才定位。表现是"更新已就绪，但写不进去"，而 SD 卡上
`.nro`、`.nro.bak`、`.nro.new` 三个文件同时存在——备份走了复制那条退路，
删源那一步失败了。用 FTP 在程序没运行时写同一个文件却完全正常，
正好排除了"我们的路径写错了"这类可能。

线索来自 GBAStation 的更新器：它在替换前先 `romfsExit()`。

我们的做法是把 `ApplyPendingUpdate` 排在 `romfsInit()` **之前**
（`CApp::Init` 的第一件事），那时文件还没被任何东西按住，一个 `rename` 就换完了。
改动只有几行，但顺序是本质的——挪到后面去就又坏了。
