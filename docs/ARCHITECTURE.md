...
### 3.7 自动更新：为什么是"下载 + 下次启动替换"

**挂着 romfs 的时候，正在运行的 NRO 文件被 FS 层按住**：删不掉、改不了名、
也打不开写。romfs 就挂在这个 NRO 文件里，所以只要 `romfsInit()` 生效着，
任何"替换自身"的尝试都注定失败。

由此决定了两件事：

1. **运行中途不替换自身**。原来的做法是下载完当场换：先把自己改名成 `.bak`，
   再把新文件挪过去，失败了再回滚。问题在于那一小段窗口里程序位置上是空的——
   回滚要是也失败，用户手上就没有能启动的 NRO 了。为一个必然失败的操作冒这个险不值得。
2. **`ApplyPendingUpdate` 排在 `CApp::Init` 的第一件事**，在 `romfsInit()` 之前。
   那时文件还没被任何东西按住，一个 `rename` 就换完了。

顺序是本质的，挪到 `romfsInit()` 后面就又坏了。

因此界面上必须在**按下下载之前**就说明"下载后需重启才能生效"——
不然用户会以为按完就装好了。

> 这一条卡了好几轮才定位，线索来自 GBAStation 的更新器（它在替换前先 `romfsExit()`）。
> 排查过程、三道校验、双源回退的具体实现见
> [开发指南 · 自动更新是怎么实现的](DEVELOPMENT.md#7-自动更新是怎么实现的)。

### 3.8 更新是会被执行的东西，所以处处保守

- **能验证就一定验证服务器证书**。地址不是 `https://` 开头时才关闭校验
  （否则必然失败），同时把这个状态显示给用户。这是退路不是常态。
- **校验 NRO 魔数**（文件偏移 0x10 处的 `NRO0`），挡住"下到一个 HTML 错误页
  然后把它装上去"。
- **三道大小校验**，每一道都安排在"还没碰用户现有程序"的位置上；
  只有装上去并验过才删备份。
- **`fclose` 的返回值必须看**。缓冲区最后一次刷盘就发生在那里，
  忽略它会把写失败伪装成一次成功的下载——曾经因此留下一个 2MB 的"完整"更新包。
